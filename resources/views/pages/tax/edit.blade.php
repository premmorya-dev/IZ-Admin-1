<form action="{{ route('tax.update') }}" id="edit-tax-form" method="POST" enctype="multipart/form-data">
    @csrf
    @method('POST')
    ...
    <div class="card shadow-lg border-0 rounded-4">
        ...
        <div class="card-body p-4">
            <div class="row g-4">
                ...
                <input type="hidden" name="tax_id" id="id_tax_id" value="{{ optional($data['tax'] ?? null)->tax_id }}">
            </div>
        </div>

        <div class="card-footer bg-light text-end rounded-bottom-4">
          
            <button type="submit" class="btn btn-success px-4 update-tax w-100">
                <i class="fas fa-save me-1"></i> Update Tax
            </button>
        </div>
    </div>
</form>
...
<script>
    document.querySelector('.update-tax').addEventListener('click', function(e) {
        e.preventDefault();
