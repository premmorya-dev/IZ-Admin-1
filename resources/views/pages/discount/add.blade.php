<form action="{{ route('discount.store') }}" id="add-discount-form" method="POST" enctype="multipart/form-data">
    @csrf
    @method('POST')
    ...
    <div class="card shadow-lg border-0 rounded-4">
        ...
        <div class="card-body p-4">
            <div class="row g-4">
                ...
                {{-- Percent --}}
                <div class="col-md-12">
                    <label class="form-label fw-semibold"><i class="fas fa-percentage me-1"></i> Percent (%)</label>
        ...
        <div class="card-footer bg-light text-end rounded-bottom-4">
            ...
            <button type="submit" class="btn btn-success px-4 save-discount w-100">
...
<script>
    document.querySelector('.save-discount').addEventListener('click', function(e) {
        e.preventDefault();
            ...
            success: function(response) {
                ...
                if (response.error === 1) {
                    ...
                    Swal.fire({
                        ...
                        // Append new discount into dropdown if inside invoice
                       if (
                            ...
                            let newDiscount = response.data; // backend must return {discount_id, name, percent}
                            let optionText = `${newDiscount.name} (${newDiscount.percent}%)`;

                            let $select = $(".discount-select");

                            let newOption = $(`<option>`)
                                .val(newDiscount.percent)
                                .attr("discount-id", newDiscount.discount_id)
                                .text(optionText);

                            $select.find("option[value='new']").before(newOption);

                            $select.val(newDiscount.percent);
