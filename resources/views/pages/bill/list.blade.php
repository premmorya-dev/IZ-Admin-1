<x-default-layout>
    ...
    <h2 class="py-3">Bills</h2>

    <!-- Accordion Container -->



    @include('pages/bill.filter')
    <div class="d-flex flex-column flex-md-row align-items-stretch align-items-md-center my-4 px-2 gap-2">
        ...
        <div id="bulk-action-section" class="d-none d-flex flex-column flex-md-row gap-2 w-100 w-md-auto">
            <a href="#" id="bulk-delete" title="Delete bill" data-bs-toggle="modal" data-bs-target="#deleteModal"
            ...
            <a href="#" id="bulk-bill-download" title="Download bill"
            ...
            <form id="download-form" method="POST" action="{{ route('bill.bulk_download') }}">
                @csrf
            </form>
        ...
        <a class="btn btn-outline-primary btn-sm w-100 w-md-auto px-4 py-2 shadow-sm fw-semibold text-uppercase d-flex align-items-center justify-content-center gap-2"
            id="add-action" href="{{ route('bill.add') }}">
    ...
    <div class="listing-grid">
        ...
        <div class="listing-header">
            <div class="checkbox-col">
                <input class="form-check-input bill_code checkbox-border-color" name="selected" type="checkbox" onclick="$('input[name*=\'selected\']').prop('checked', this.checked);" value="" id="flexCheckDefault">
                <span class="d-block d-md-none text-center fw-bold ms-1"> Select All </span>
            </div>

            <div class="col sortable"><i data-lucide="file-text"></i> @sortablelink('bill_number','bill #') </div>
            <div class="col sortable"><i data-lucide="check-circle"></i> @sortablelink('bill_status','Status') </div>
        ...
        @if ( $data['bill'] )
        @foreach ($data['bill'] as $index => $bill )


        <div class="listing-row">

            <div class="checkbox-col">
                <input class="form-check-input selected checkbox-border-color" name="selected[]" type="checkbox" value="{{ $bill->bill_code }}" id="flexCheckDefault">
            </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    Are you sure you want to delete the bills?
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                    <button type="button" bill-code="" class="btn btn-danger" id="confirmed-single-delete">Delete</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="multiDeleteModal" tabindex="-1" aria-labelledby="multiDeleteModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    Are you sure you want to delete the all selected bills?
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                    <button type="button" bill-code="" class="btn btn-danger" id="confirmed-multi-delete">Delete</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="emailConfirmModal" tabindex="-1" aria-labelledby="emailConfirmModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white rounded-top-3">
                    <h5 class="modal-title">Send Bill Emails</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    Are you sure you want to send selected bills to vendors via email?
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
  ...
  <script>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('change', '.selected, .bill_code', function() {
            ...
            $('#confirmed-multi-delete').on('click', function(e) {
                ...
                try {
                    $.ajax({
                        url: "{{ route('bill.bulk_delete') }}",
                        data: {
                            bills_code: bills_code
                        },
                        type: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}'
            ...
            $('#bulk-bill-download').on('click', function(e) {
                e.preventDefault();
                let selectedbills = $(".selected:checked");
                let form = $('#download-form');

                form.find('input[name="bills_code[]"]').remove(); // clear old values
