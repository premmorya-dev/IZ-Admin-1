<x-default-layout>
    ...
    <form action="#" id="bill-generate" method="POST">
        ...
        <div class="page-header-fixed mb-3 d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex justify-content-between">
                <a href="#" bill-code="{{ $data['bill']->bill_code }}" title="Preview bill" class="bill-view-model btn btn-sm btn-outline-primary"><i class="fa-regular fa-eye text-default"></i> </a>
                <a class="btn btn-sm btn-outline-primary" href="{{ route('bill.download',['bill_code' => $data['bill']->bill_code ]) }}?preview=true" target="__blank" title="Print bill"><i class="fa-solid fa-print  text-defaults"></i> </a>
                <a class="btn btn-sm btn-outline-primary" href="{{ route('bill.download',['bill_code' => $data['bill']->bill_code ]) }}" title="Download bill"><i class="fa-solid fa-download text-defaults"></i> </a>
            </div>

            @include('pages/bill/actions.update_bill_action')
        </div>
        ...
        <div class="row">
            ...
            <!-- bill Number with Mode -->
            <div class="col-md-3 mt-3">
                <label for="bill_number" class="form-label d-flex justify-content-between align-items-center text-danger">
                    <span>Bill Number *</span>
                </label>
                <input type="text" class="form-control form-control-sm" id="bill_number" value="{{ old('bill_number', $data['bill']->bill_number ) }}" name="bill_number" placeholder="Auto-generated" readonly>
            </div>
            ...
            <div class="col-md-3 mt-3">
                ...
                <select name="currency_code" id="currency_code" class="form-select">
                    ...
                    <option value="{{ $currency->currency_code }}" {{ old('currency_code',$data['bill']->currency_code) == $currency->currency_code ? 'selected' : '' }}>
            ...
            <div class="col-md-3 mt-3">
                ...
                <select class="form-select" id="template_id" name="template_id">
                    ...
                    <option value="{{ $template->template_id }}" {{ old('template_id',$data['bill']->template_id) == $template->template_id ? 'selected' : '' }}>{{ $template->template_name }}</option>
                    @endforeach
                </select>
            </div>
                        ...
                        <div class="mt-3">
                            ...
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="is_recurring" name="is_recurring"
                                    {{ old('is_recurring', $data['bill']->is_recurring == 'Y'  ? 'checked' : '') }}>
                ...
                <div class="row mt-5">
                    <div class="col-md-6 mt-3">
                        ...
                        <div class="input-group">
                            <input type="text" id="bill_date" class="form-control" name="bill_date" value="{{ old('bill_date', $data['bill']->bill_date )  }}" placeholder="Select bill Issue Date">
                    ...
                    <div class="col-md-6 mt-3">
                        ...
                        <div class="input-group">
                            <input type="text" id="due_date" class="form-control" name="due_date" value="{{ old('due_date', $data['bill']->due_date )  }}" placeholder="Select bill Due Date">
                            <span class="input-group-text"><i class="fas fa-calendar-alt"></i></span>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6 mt-3">
                        ...
                        <select name="supply_source_state_id" id="supply_source_state_id" class="form-select">
                            ...
                            <option value="{{ $state->state_id }}" {{ old('supply_source_state_id',$data['bill']->supply_source_state_id) == $state->state_id ? 'selected' : '' }}>
                                {{ $state->state_name }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 mt-3">
                        ...
                        <select name="destination_source_state_id" id="destination_source_state_id" class="form-select">
                            <option value="">Please Select</option>
                            @foreach($data['states'] as $state)
                            <option value="{{ $state->state_id }}" {{ old('destination_source_state_id',$data['bill']->destination_source_state_id) == $state->state_id ? 'selected' : '' }}>
                                {{ $state->state_name }}
                            </option>
                            @endforeach
                        </select>
            ...
            <div class="container my-4">
                ...
                <input type="hidden" name="bill_code" value="{{ $data['bill']->bill_code }}" id="bill_code">
            ...
            <div class="col-md-12 mt-5">
                ...
                <div class="col-12">
                    ...
                    <textarea id="id_bill_notes" name="notes" class="form-control" placeholder="Enter Notes">{{ old('notes', $data['bill']->notes ) }}</textarea>
                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            var address = `{!! $data['vendor_details_html'] !!}`;
            var vendor_id = `{{ $data['bill']->vendor_id }}`;

            if (address) {
                $('#vendorAddress').html(address).show();
                $('#vendorSearchBox').hide();
                $('#vendorActionBtn').attr('status', 'false')
                $('#vendorActionBtn').text('✏️ Change Vendor').show();
